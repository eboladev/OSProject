...
void DateWidget::getDate()
{
    ...
    QString solarstr = QString::number(curyear)+tr("年")+QString::number(curmonth)+tr("月")+QString::number(curday)+tr("日");
   ...
   // calendar.ctcl_solar_to_lunar(2012,6,1,&cal);
    ...
    //qDebug()<<holiday;
   // qDebug()<<cal.ganzhi<<cal.cmonth<<cal.cday;      //阴历年
    QString lunarstr =cal.ganzhi +tr("年")+cal.cmonth+cal.cday;
