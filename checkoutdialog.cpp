...
CheckOutDialog::CheckOutDialog(QWidget *parent,QSqlRelationalTableModel *pModel,bool pIsAdd,int pIndex) :
    QDialog(parent),
...
void CheckOutDialog::initView(){
    ...
    ui->lineEdit_claimer->setEnabled(false);
    ...
    if(this->index != -1){
        ...
        query.prepare("select a.cardNo,a.name,a.department,a.gender,a.post,b.amount,"
                      "b.auditor,b.outDate,b.name,b.operationTime,b.operator,b.provider,b.rate,"
                      "b.standard,b.type,b.unit,b.warehouse from employee_info a,check_out b where a.cardNo = b.claimer and b.id = ?");
        ...
        if(query.first()){
            ...
            ui->lineEdit_claimer->setText(query.value(1).toString());
            ...
            ui->dateEdit_outDate->setDate(query.value(7).toDate());
...
void CheckOutDialog::on_btn_save_clicked()
{
    QSqlQuery query(globaldb);
    if(this->isAdd){
        query.prepare("insert into check_out values (NULL,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        query.addBindValue(ui->lineEdit_cardNo->text());
    }
    else{
        query.prepare("update check_out set type = ?,name = ?,standard = ?, unit =?,rate=?,amount=?,provider=?,"
                      "warehouse=?,outDate=?,auditor=?,operator=?,operationTime=?");
    }
...
void CheckOutDialog::on_toolButton_clicked()
{
...
void CheckOutDialog::doubleClickRecord(QString cardNo){
    QSqlQuery query(globaldb);
    ...
    if( query.first()){
        ...
        ui->lineEdit_claimer->setText(query.value(0).toString());
...
void CheckOutDialog::on_toolButton_storage_clicked()
{
    storageDialog = new StorageChooseDialog(this);
    connect(storageDialog,SIGNAL(sendStorageChoosed(int)),this,SLOT(doubleClickStorage(int)));
    storageDialog->exec();
}

void CheckOutDialog::doubleClickStorage(int id){
    QSqlQuery query(globaldb);
    query.prepare("select type,name,standard,unit,rate,amount,warehouse,provider from storage where id = ?");
    query.addBindValue(id);
    query.exec();
    if(query.lastError().isValid())
        qDebug()<<query.lastError();
    if( query.first()){
        ui->comboBox_type->setCurrentIndex(ui->comboBox_type->findText(query.value(0).toString()));
        ui->lineEdit_name->setText(query.value(1).toString());
        ui->comboBox_standard->setCurrentIndex(ui->comboBox_standard->findText(query.value(2).toString()));
        ui->comboBox_unit->setCurrentIndex(ui->comboBox_unit->findText(query.value(3).toString()));
        ui->lineEdit_rate->setText(query.value(4).toString());
        ui->comboBox_warehouse->setCurrentIndex(ui->comboBox_warehouse->findText(query.value(6).toString()));
        ui->lineEdit_provider->setText(query.value(7).toString());
    }
    query.clear();
    query.finish();
}
